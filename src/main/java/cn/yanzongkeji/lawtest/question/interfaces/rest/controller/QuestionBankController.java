...
import cn.yanzongkeji.lawtest.question.interfaces.rest.request.QuestionBankCreateRequest;
...
public class QuestionBankController {
  ...
  @PostMapping
  @Operation(summary = "创建空题库")
  public ResponseEntity<QuestionBankResponse> create(
      @RequestBody QuestionBankCreateRequest request) {
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(QuestionBankResponse.from(commands.createQuestionBank(request.name())));
  }

  @GetMapping
  @Operation(summary = "分页查询题库")
  public PageResponse<QuestionBankResponse> list(
    ...
    return new PageResponse<>(
        result.items().stream()
            .map(
                x ->
                    QuestionBankResponse.from(
                        (cn.yanzongkeji.lawtest.question.domain.model.QuestionBank) x))
            .toList(),
        result.page(),
        result.size(),
        result.total());
