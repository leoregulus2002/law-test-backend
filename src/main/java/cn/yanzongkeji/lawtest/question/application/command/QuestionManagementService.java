...
public class QuestionManagementService implements QuestionManagementUseCase {
  ...
  @Override
  @Transactional
  public QuestionBank createQuestionBank(String name) {
    QuestionBank bank =
        QuestionBank.create("系统创建", new QuestionBankCode(systemCreationCode()), name);
    QuestionBankRepository.ImportResult saved = banks.createIfAbsent(bank);
    return banks
        .findById(saved.questionBankId())
        .orElseThrow(() -> new IllegalStateException("题库创建失败"));
  }
  ...
  public Question create(long bankId, QuestionCommand command) {
    ...
    if (banks.findById(id).isEmpty()) throw new QuestionBankNotFoundException(bankId);
  ...
  public void changeStatuses(List<Long> questionIds, QuestionStatus status) {
    for (Long questionId : validIds(questionIds)) changeStatus(questionId, status);
  }
  ...
  public void deleteQuestions(List<Long> questionIds) {
    for (Long questionId : validIds(questionIds)) deleteQuestion(questionId);
  }
  ...
  private static Question toQuestion(QuestionBankId bankId, QuestionCommand command) {
    try {
      List<QuestionOption> options =
          command.options().stream().map(o -> new QuestionOption(o.label(), o.content())).toList();
      return Question.create(
          bankId,
          new QuestionNumber(command.number()),
          command.stem(),
          options,
          new AnswerKey(command.answers()),
  ...
  private static String systemCreationCode() {
    try {
      byte[] digest =
          MessageDigest.getInstance("SHA-256")
              .digest(("SYSTEM:" + UUID.randomUUID()).getBytes(StandardCharsets.UTF_8));
